...
button = tkinter.Button(root,text='计算BMI',command=BMI)
button.place(x=10,y=250,height=30,width=80)

weight=tkinter.StringVar(root)
entryBmi=tkinter.Entry(root,width=150,textvariable=label1)
entryBmi.place(x=70,y=20,width=100,height=40)


height=tkinter.StringVar(root)
entryBmi=tkinter.Entry(root,width=150,textvariable=label2)
